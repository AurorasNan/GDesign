...
class FPN(ScriptModuleWrapper):
    ...
    __constants__ = ['interpolation_mode', 'num_downsample', 'use_conv_downsample', 'relu_pred_layers',
                     'lat_layers', 'pred_layers', 'downsample_layers', 'relu_downsample_layers','adaptive_pool_output_ratio']
    ...
    def __init__(self, in_channels):
        ...
        self.adaptive_pool_output_ratio = cfg.fpn.pool_ratios
        self.high_lateral_conv = nn.ModuleList()
        self.high_lateral_conv.extend([nn.Conv2d(in_channels[-1], 256, 1) for k in range(len(self.adaptive_pool_output_ratio))])
        self.high_lateral_conv_attention = nn.Sequential(nn.Conv2d(256*(len(self.adaptive_pool_output_ratio)), 256, 1),nn.ReLU(), nn.Conv2d(256,len(self.adaptive_pool_output_ratio),3,padding=1))
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        h, w = convouts[-1].size(2), convouts[-1].size(3)
        # Ratio Invariant Adaptive Pooling
        AdapPool_Features = [F.interpolate(mod(F.adaptive_avg_pool2d(convouts[-1],output_size=(max(1,int(h*self.adaptive_pool_output_ratio[j])), max(1,int(w*self.adaptive_pool_output_ratio[j]))))),size=(h,w), mode='bilinear', align_corners=True) for j, mod in enumerate(self.high_lateral_conv)]
        # print("w:", self.high_lateral_conv[0])
        # AdapPool_Features = [F.upsample(
        #     self.high_lateral_conv[j](F.adaptive_avg_pool2d(convouts[-1],
        ...
        adap_pool_fusion = torch.zeros(1, device=convouts[0].device)
        for i in range(len(self.adaptive_pool_output_ratio)):

            m = adap_pool_fusion + torch.unsqueeze(fusion_weights[:, i, :, :], dim=1) * AdapPool_Features[i]
            adap_pool_fusion = m
        ...
        for lat_layer in self.lat_layers:
            lat_layer.modules()
            j -= 1
            # print("size:", convouts[j].size())
            if j < len(convouts) - 1:
                _, _, h, w = convouts[j].size()
                # print("size:",convouts[j].size())
                x = F.interpolate(out[j + 1], size=(h, w), mode=self.interpolation_mode, align_corners=False)
            x = x + lat_layer(convouts[j])
            if j == 2: x = x + adap_pool_fusion
            out[j] = x
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        #让定义的backbone里面的除Conv之外的层不参与梯度运算
        if cfg.freeze_bn:
            self.freeze_bn()
